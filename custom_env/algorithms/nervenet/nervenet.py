...
class NerveNet(nn.Module):
    def __init__(self,
        j_obs_dim, target_obs_dim, root_obs_dim,
        ...
        self.root_enc = ObservationEncoder(root_obs_dim, obs_enc_hidden_dim, hidden_state_dim)
    ...
    def forward(self, target_obs, j_obs, root_obs, senders, receivers, actuatable_nodes):
        target_hidden = self.target_enc(target_obs)
        j_hidden = self.target_enc(j_obs)
        root_hidden = self.target_enc(root_obs)
